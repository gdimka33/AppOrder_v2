import os
from справочники import (
    СПРАВОЧНИК_ЗВАНИЙ, СПРАВОЧНИК_ДОЛЖНОСТЕЙ, СПРАВОЧНИК_ПОДРАЗДЕЛЕНИЙ, СПРАВОЧНИК_ПОСТОВ, СПРАВОЧНИК_ТИПОВ_НАРЯДОВ, СПРАВОЧНИК_ОФИЦЕРОВ, СПРАВОЧНИК_КУРСАНТОВ, загрузить_все_справочники
)

загрузить_все_справочники()
...
# Попытка импортировать виджет графика из модуля учёта службы (если присутствует)
from Служебный_наряд.кл_ПриказСлужебныйНаряд import ПриказСлужебныйНаряд
try:
    from Учет_службы.виджиты.кл_График import График
except Exception:
    График = None
...
class ГлавноеОкно(tk.Tk):
    def __init__(self):
        ...
        # --- Раздел: Учёт службы ---
        группа_учет_службы = ttk.LabelFrame(фрейм_меню, text="Учёт службы", padding=5, style='Menu.TLabelframe')
        группа_учет_службы.pack(fill='x', padx=5, pady=(8,2))

        self.кнопки["График"] = tk.Button(
            группа_учет_службы,
            text="График",
            command=lambda: self.открыть_вкладку("График"),
            **self.стили.button_config
        )
        self.кнопки["График"].pack(pady=2, fill='x')
        self.состояния_вкладок["График"] = "не_начато"
    ...
    def открыть_вкладку(self, название):
        ...
        if название not in self.вкладки:
            ...
            elif название == "График":
                новая_вкладка = ttk.Frame(self.фрейм_с_вкладками_контент)
                self.фрейм_с_вкладками_контент.add(новая_вкладка, text=название)
                self.вкладки[название] = новая_вкладка
                # Если виджет График доступен — создаём его внутри вкладки
                if График:
                    try:
                        виджет = График(новая_вкладка)
                    except Exception:
                        pass
            else:
                новая_вкладка = ttk.Frame(self.фрейм_с_вкладками_контент)
